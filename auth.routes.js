...
router.post('/setup', (req, res) => {
  const count = db.prepare('SELECT COUNT(*) c FROM users').get().c;
  if (count > 0) return res.status(403).json({ error: 'Setup has already been completed for this system.' });
  ...
  const result = db.prepare('INSERT INTO users (email, password_hash, role, employee_id) VALUES (?,?,?,?)')
...
router.post('/login', (req, res) => {
  ...
  const employee = db.prepare('SELECT * FROM employees WHERE id = ?').get(user.employee_id);
  const token = signToken(user);
  res.json({
    token,
